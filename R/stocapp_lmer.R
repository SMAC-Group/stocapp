...
stocapp.lmerMod <- function(object, thetastart=NULL, control=list(...), extra_param = FALSE, ...){
  # controls
  ...
  tmp_object <- eval(cl,env_stocapp)
  ...
  while(test_theta > control$tol && k < control$maxit){
    ...
    for(h in seq_len(control$H)){
      ...
      tmp <- getParam(eval(cl,env_stocapp),extra_param)
...
#' @export
setMethod("stocapp", signature = className("lmerMod", "lme4"),
          definition = stocapp.lmerMod)
...
#' @description simulation method for class \linkS4class{StocappLmer}
