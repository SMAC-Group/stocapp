...
stocapp.nls <- function(object, thetastart=NULL, control=list(...), extra_param = FALSE, ...){
  # controls
  ...
  for(i in x_name) assign(i, data[[i]], env_stocapp)
  assign("y", data[[y_name]], env_stocapp)
  cl$formula[[2]] <- quote(y)
  cl$data <- quote(env_stocapp)
...
#' @export
setMethod("stocapp", signature = className("nls","stats"),
          definition = stocapp.nls)
...
#' @description simulation method for class \linkS4class{StocappNls}
